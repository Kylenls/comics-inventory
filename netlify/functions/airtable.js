...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ id: record.id, fields: record.fields }) };
    }
    ...
    if (action === 'upsert') {
      ...
      if (!series) {
        const record = await base(useTable).create(fields);
        return { statusCode: 200, headers, body: JSON.stringify({ id: record.id, fields: record.fields, _action: 'created' }) };
      }
